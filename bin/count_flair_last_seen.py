...
flairs = {}
most_recent = {}
flair_choices = subreddit.flair.templates
...
flair_list = list(subreddit.flair(limit=None))
...
for f in flair_list:
    ...
    if fclass in flairs and flairs[fclass] <= low_floir_count:
        ...
        user = f['user']
        print('reading last seen for flair/count/user: {}/{}/{}'.format(fclass, flairs[fclass], user.name))
        try:
            c = list(user.comments.new(limit=1))
            ctime = c[0].created_utc if len(c) else 0
        except:
            print('error. skipping {} comments'.format(user.name))
            ctime = 0

        try:
            s = list(user.submissions.new(limit=1))
        ...
        if last_seen > most_recent[fclass]['time']:
            most_recent[fclass] = {'time': last_seen, 'user': user.name}
